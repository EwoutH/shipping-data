...
dates = [f"2022-10-{day}" for day in range(14,18)]

# Read first DataFrame from Pickle
comb_df = pd.read_pickle(f"pickles/connections_{dates[0]}.pickle")
...
for date in dates[1:]:
    ...
    new_df = pd.read_pickle(f"pickles/connections_{date}.pickle")
...
comb_df.to_pickle(f"pickles/connections_combined.pickle")
comb_df.to_csv(f"data/connections_combined.csv")
